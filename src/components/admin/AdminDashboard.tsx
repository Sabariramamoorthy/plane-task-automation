...
function buildDrafts(users: AdminDashboardData["users"]) {
...
export function AdminDashboard({ initialData }: { initialData: AdminDashboardData }) {
  ...
  const [drafts, setDrafts] = useState<Record<string, LimitDraft>>(() => buildDrafts(initialData.users));

  useEffect(() => {
    setData(initialData);
    setDrafts(buildDrafts(initialData.users));
  }, [initialData]);
              ...
              <div key={item.id} className="rounded-md border border-zinc-200 p-4">
                <div className="mb-3 flex flex-wrap items-center justify-between gap-2">
                  <div>
                    ...
                    <p className="text-xs text-zinc-500">Instances: {item.instanceCount}</p>
                    <p
                      className={
